...
class DoubleBert(nn.Module):
    ...
    def forward(self, x_1 , x_2):
        ...
        similarity_score = self.activation(similarity_score)
        similarity_score = self.dropout(similarity_score)
        similarity_score = self.normalize(similarity_score)
        
        
        similar = self.similar(similarity_score)
        
        logit_score = self.last_output(similarity_score)
        soft_max = self.softmax(logit_score)
